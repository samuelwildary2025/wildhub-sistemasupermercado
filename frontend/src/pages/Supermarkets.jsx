...
import { getSupermarkets, createSupermarket, updateSupermarket, deleteSupermarket, getSupermarketIntegrationToken, resetSupermarketPassword } from '../services/api'
...
const Supermarkets = () => {
  ...
  // Carrega o token de integração quando o modal de detalhes é aberto
  useEffect(() => {
    const fetchToken = async () => {
      ...
      try {
        ...
        setIntegrationToken('')
        const res = await getSupermarketIntegrationToken(selectedSupermarket.id)
        const token = res?.data?.access_token || res?.access_token
        if (token) setIntegrationToken(token)
      } catch (e) {
        console.error('Erro ao obter token de integração:', e)
      } finally {
                    ...
                    <div key={supermarket.id} className="card hover:shadow-lg transition-shadow">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <h3 className="font-semibold text-white">{supermarket.nome}</h3>
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-center space-x-2">
                    <button
                      ...
                      className="flex items-center space-x-1 px-3 py-2 bg-dark-700 hover:bg-dark-600 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded transition-colors"
                    >
                      <ChevronLeft size={16} />
                      <span>Anterior</span>
                    </button>
              ...
              <div className="p-2 rounded bg-dark-900 border border-dark-700 text-sm">
                <p className="text-dark-200 mb-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  <span>
                    ...
                    {isLoadingIntegrationToken
                      ? 'carregando…'
                      : integrationToken
                      ? `${integrationToken.substring(0, 32)}…`
                      : 'não disponível'}
                  </span>
                  <button
                    onClick={() => {
                ...
                <p className="text-dark-400 text-xs">
                  Este token identifica o supermercado para o agente.
                </p>
              </div>
