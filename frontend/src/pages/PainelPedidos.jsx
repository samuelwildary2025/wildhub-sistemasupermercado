    ...
    <div className="min-h-screen">
      ...
      <div className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <KpiCard title="Faturados" value={stats.faturados} icon={CheckCircle} color="text-green-600 dark:text-green-400" />
          <KpiCard title="Valor Faturado" value={formatCurrency(stats.valorTotal)} icon={DollarSign} color="text-green-600 dark:text-green-400" />
        </div>
              ...
              <div className="space-y-3">
                {pendentesPedidos.map(pedido => {
                  ...
                  return (
                    <div
                      key={pedido.id}
                      className={`w-full flex items-center justify-between py-3 px-3 rounded-lg bg-gray-100 dark:bg-dark-800 hover:bg-gray-200 dark:hover:bg-dark-700 border shadow-sm cursor-pointer ${
                        selectedPedido?.id === pedido.id
                          ? 'ring-2 ring-yellow-400 border-yellow-600'
                          : 'ring-1 ring-transparent border-gray-300 dark:border-dark-700'
                      }`}
                    >
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-yellow-600 dark:text-yellow-400 font-semibold">
                          {formatCurrency(orderTotal(pedido))}
                        </span>
                        <button
                          onClick={e => {
                            e.stopPropagation()
                            handleStatusChange(pedido.id, 'faturado')
                          }}
            ...
            {concluidosPedidos.length === 0 ? (
              ...
              <div className="space-y-3">
                {concluidosPedidos.map(pedido => (
                  <PedidoCard key={pedido.id} pedido={pedido} onStatusChange={handleStatusChange} onOpen={() => openDetails(pedido)} />
                ))}
              </div>
            )}
          </div>
              ...
              <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-dark-700">
                <h3 className="text-gray-900 dark:text-white font-semibold text-lg">
                  {`Pedido de ${
                    ...
                    selectedPedido?.client_name ||
                    'Cliente'
                  }`}
                </h3>
                <button
                  className="text-gray-500 dark:text-dark-400 hover:text-gray-900 dark:hover:text-white"
                ...
                <div className="space-y-4 lg:col-span-2">
                  ...
                  <div className="bg-gray-50 dark:bg-dark-900 rounded-lg p-4 border border-gray-200 dark:border-dark-700">
                    ...
                    <div className="space-y-2">
                      {(Array.isArray(selectedPedido?.itens)
                        ...
                        return (
                          <div
                            key={idx}
                            className="flex items-center justify-between p-3 bg-white dark:bg-dark-800 rounded border border-gray-200 dark:border-dark-700"
                          >
                            <div>
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      <span className="text-green-600 dark:text-green-400 font-extrabold text-lg">
                        {formatCurrency(orderTotal(selectedPedido))}
                      </span>
                    </div>
                  </div>
                    ...
                    {chatMessages.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {chatMessages.map(m => (
                          <div
                            key={m.id}
                            className="bg-gray-100 dark:bg-dark-700 text-gray-800 dark:text-dark-100 text-sm p-2 rounded"
                          >
                            {m.text}
                          </div>
                        ))}
