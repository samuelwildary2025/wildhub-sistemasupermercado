...
_numero_pedido_migration_done = False # Mantemos apenas esta migração, que é mais estável.
...
def _ensure_numero_pedido_column(db: Session) -> None:
    ...
    try:
        ...
        # Se houver erro de tabela inexistente ou outra falha na migração, apenas logamos
        print(f"Alerta: Falha na migração numero_pedido (ignorado): {e}")
        pass
...
def update_pedido_por_telefone(
    ...
    _ensure_numero_pedido_column(db)
    # ⚠️ A FUNÇÃO _ensure_foi_alterado_column FOI REMOVIDA DAQUI PARA EVITAR O CRASH
    # E PASSADA PARA O main.py E ROUTERS DE LISTAGEM
